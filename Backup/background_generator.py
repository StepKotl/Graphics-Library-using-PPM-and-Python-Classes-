...
thing = frame((2560, 1440), [0, 0, 0])

Name = text((int(thing.size[0] / 2 - int((textSize * scale) / 2)), int(thing.size[1] / 2) - int((5 * scale) / 2)), scale, [109, 204, 106], writtenText)
border = square((0, 0), 7, [77, 161, 74], (thing.size[0] - 1, thing.size[1] - 1), False)

lines = []

for x in range(int(math.floor((thing.size[0]) / (textSize + 2)))):
